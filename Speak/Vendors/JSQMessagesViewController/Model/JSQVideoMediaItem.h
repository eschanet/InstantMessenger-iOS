...
/**
 *  The `JSQESVideoMediaItem` class is a concrete `JSQMediaItem` subclass that implements the `JSQMessageMediaData` protocol
 *  and represents a video media message. An initialized `JSQESVideoMediaItem` object can be passed
 *  to a `JSQMediaMessage` object during its initialization to construct a valid media message object.
 *  You may wish to subclass `JSQESVideoMediaItem` to provide additional functionality or behavior.
 */
@interface JSQESVideoMediaItem : JSQMediaItem <JSQMessageMediaData, NSCoding, NSCopying>
 ...
 *
 *  @return An initialized `JSQESVideoMediaItem` if successful, `nil` otherwise.
 *
 *  @discussion If the video must be downloaded from the network,
 *  you may initialize a `JSQESVideoMediaItem` with a `nil` fileURL or specify `NO` for
