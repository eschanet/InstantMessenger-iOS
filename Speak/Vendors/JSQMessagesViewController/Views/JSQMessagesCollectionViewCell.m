...
- (void)ES_handleLongTapGesture:(UILongPressGestureRecognizer *)tap;
...
- (void)awakeFromNib
{
    ...
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(ES_handleLongTapGesture:)];
...
- (void)ES_handleLongTapGesture:(UILongPressGestureRecognizer *)tap {
    ...
    if (CGRectContainsPoint(self.messageBubbleContainerView.frame, touchPt)) {
        [self.delegate messagesCollectionViewCellDidLongTapMessageBubble:self];
    }
}
